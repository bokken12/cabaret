...
impl Cabaret {
    ...
    pub fn base(&self, change: &ChangeId) -> Result<Base> {
        ...
        match deepest.as_slice() {
            [] => Ok(Base::Empty),
            [(_, revision)] => Ok(Base::Real(Revision(*revision))),
            _ => self.synthesize(&deepest),
        }
    }

    /// The revision of `parent` that `change`'s tip most recently incorporated.
    fn incorporated(&self, change: &ChangeId, tip: ObjectId, parent: &ChangeId) -> Result<ObjectId> {
        let parent_tip = self.tip(parent)?;
        let bases = self.repo.merge_bases_many(tip, &[parent_tip])?;
    ...
    fn without_dominated<'a>(
        &self,
        incorporated: &[(&'a ChangeId, ObjectId)],
    ) -> Result<Vec<(&'a ChangeId, ObjectId)>> {
    ...
    fn is_ancestor(&self, ancestor: ObjectId, descendant: ObjectId) -> Result<bool> {
        Ok(self.repo.merge_bases_many(ancestor, &[descendant])?.iter().any(|base| *base == ancestor))
    }
    ...
    fn synthesize(&self, incorporated: &[(&ChangeId, ObjectId)]) -> Result<Base> {
        ...
        let mut tree = self.repo.find_commit(merged)?.tree_id()?.detach();
        ...
        for (index, &(parent, revision)) in rest.iter().enumerate() {
            ...
            tree = merge.tree_merge.tree.write()?.detach();
        ...
        Ok(Base::Synthetic { revision: Revision(self.synthetic_commit(tree, revisions)?), conflicts })
    }

    /// Markers must outsize any markers already committed at the conflicted paths, or nested
    /// conflicts would be ambiguous.
    fn marker_size(&self, tree: ObjectId, conflicts: &[String]) -> Result<NonZeroU8> {
    ...
    fn synthetic_commit(&self, tree: ObjectId, parents: Vec<ObjectId>) -> Result<ObjectId> {
        let signature =
