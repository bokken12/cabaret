...
test("comment fails on a change that does not exist", async () => {
  ...
  expect(await repo.cabaret("comment", "--change", "ghost", "anyone home?")).toEqual({
    stdout: "",
    stderr: 'change does not exist: "ghost"; run `cab create`, or `cab fetch` to import open forge changes\n',
    exitCode: 1,
  });
  expect(await repo.cabaret("dev", "log", "ghost")).toEqual({ stdout: "", stderr: "", exitCode: 0 });
});
