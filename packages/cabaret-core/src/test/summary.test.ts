...
test("a landed change reports its merge, and a moved-base review counts as left", async () => {
  ...
  const entries = [
    ...created("main", "1"),
    entry({ kind: "set-forge", forge: parseForgeLocator("github.com/test-org/widgets"), request: forgeRequestId(7) }),
  ...
  expect(await summarizeChange(backend, feature, entries, alice)).toEqual({
    ...
    parent: "main",
    owner: alice,
    forgeRequest: { forge: "github.com/test-org/widgets", request: 7 },
    landed: fake("5"),
    base: fake("1"),
    tip: fake("4"),
    reviewLeft: ["a.ts", "b.ts"],
