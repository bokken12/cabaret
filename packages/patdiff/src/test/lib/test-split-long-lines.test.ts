/** Port of OCaml's [patdiff/test/src/test_split_long_lines.ml]. */

import { describe, expect, it } from "vitest";
import { patdiff } from "./_helpers.js";

// OCaml uses [String.lstrip] on the input, dropping the leading newline.
const prev = `_no_inline node transform(input :  u32x4)
  returns out :  u32x4
vars
  x0 :  u32[4],
  x1 :  u32[3],
...
const next = `_no_inline node transform'(input' :  u32x4)
  returns out' :  u32x4
vars
  x0' :  u32[4],
  x1' :  u32[3],
...
describe("split long lines", () => {
  it("~split_long_lines:false", () => {
  ...
  it("~split_long_lines:true", () => {
    expect(patdiff({ prev, next, extraFlags: ["-split-long-lines"] })).toMatchInlineSnapshot(`
      "(fg:red)------ (fg:default +bold)prev/file(-weight)
      (fg:green)++++++ (fg:default +bold)next/file(-weight)
      (bg:gray fg:black)@|(bg:default fg:default +bold)-1,11 +1,10(-weight) ============================================================
      (bg:red fg:black)-|(off fg:gray-12)_no_inline node(fg:red) transform(fg:gray-12)((fg:red)input(fg:gray-12) :  u32x4)(fg:default)
