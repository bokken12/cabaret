...
When you violate this, editing the history, git can no longer tell which of the original commits line up with your new cleaned-up versions. Unless the branch is exclusive to you (unlikely in Cabaret if others are pushing their code review on it), this forces everyone interacting with your branch to do "force-push" style operations, where they either abandon their local copy or the remote changes. It makes merge conflicts more prevalent and more difficult to resolve. This is bad for a version control system.
