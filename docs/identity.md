...
Declaring an account by hand goes through the same scheme without spelling it: `cabaret config alias github add alice` stores `github:alice`, and each forge has the same `add`, `remove`, and `clear` under its name.
